...
/**
 * tools/spatial_shared/spatial_tool.js — the shared gridded FAO-56 factory.
 *
 * Draw or upload an area, pick a period and crop/management settings, sign in
 * to Earth Engine; soil + vegetation index + weather are pulled onto one grid,
 * engine.js runs once per pixel (single thread), and the result is mapped.
 *
 * This is the generic UI/orchestration only. The parts that differ by scale —
 * the vegetation/soil registries and the Earth Engine acquisition (collectGrid)
 * — are INJECTED via config by each tool (tools/field-scale, tools/mesoscale),
 * so a change to one tool's data layer cannot reach the other. The FAO-56
 * physics and the map/results live in the shared pure modules alongside this
 * file. See TOOLS.md.
 *
...
import { sourceIndex } from './veg_index.js';
...
export function createSpatialTool(config) {
  ...
  /* ── Sidebar: weather (atmosphere) source ───────────────────────────────
     Offered only when a tool has more than one weather source (Field Scale:
     GRIDMET or Open-Meteo). With a single source (Mesoscale: GRIDMET) the group
     is omitted, so nothing changes there. The selected source drives the
     'Atmosphere' inputs the same way the dataset drives 'Plant' and the soil
     source drives 'Soil'. */
  let gWeather = null;
  const weatherNote = el('div', { class: 'hint', style: { marginTop: '0.4rem', lineHeight: '1.4' } }, state.weatherSource ? state.weatherSource.note : '');
  if (weatherList.length > 1) {
    const weatherSel = selectInput({ options: weatherList.map((w) => ({ value: w.id, label: w.label })), value: state.weatherSource.id, onChange: onWeather });
    gWeather = group('Weather', { open: true });
    gWeather.body.append(ctrl('Source', weatherSel.el), weatherNote);
  }

  /* ── Sidebar: soil source ───────────────────────────────────────────────
     Offered only when a tool has more than one soil source (Field Scale:
  ...
  if (config.boundaryLevels && config.boundaryLevels.length) {
    ...
    gBoundary = group('Region boundary', { open: true });
  ...
  /* EVI endpoints: bare soil ≈ 0.15, full canopy at 0.80 (dense canopy peaks
     ~0.7–0.9 and Landsat EVI can reach 1.0 on bright pixels). The rescale
     clamps, so any VI ≥ the full-cover value saturates to Kcb max.
  ...
  /* Order: connect (Earth Engine) → when (Period) → where (Region, mesoscale)
     → the three data layers Weather / Soil / Vegetation index, with the VI
     dataset last so it sits next to the VI→Kcb settings in Crop → Management. */
  wb.sidebar.append(...[gEE.el, gArea.el, gBoundary && gBoundary.el, gWeather && gWeather.el, gSoil && gSoil.el, gData.el, gCrop.el, gMgmt.el].filter(Boolean));
  ...
  /* Optional grid-resolution selector (Mesoscale: 500 m–4 km; a coarser grid
     trades detail for area). Absent when config.resolutions is unset — Field
     Scale sources each carry one resolution (e.g. Sentinel-2 at 10 m or 30 m
     are two entries), so the choice is the Source dropdown itself. */
  ...
  function onDataset(id) {
    const src = config.getVegSource(id);
    state.vegSource = src;
  ...
  /* Switching weather source only swaps which atmosphere product the run pulls;
     the grid, soil and VI are unaffected. */
  function onWeather(id) {
    state.weatherSource = config.getWeatherSource(id);
  ...
  async function run() {
    ...
    try {
      ...
      const data = await config.collectGrid(ee, {
