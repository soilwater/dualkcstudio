...
/**
 * modes/eto-calculator/index.js — a small utility mode: upload a weather
...
function create() {
  ...
  const latIn = numInput({ min: -90, max: 90, step: 0.0001, onChange: sync });
  const elevIn = numInput({ min: -430, max: 6000, step: 1, onChange: sync });
  const gSite = group('Site', { open: true });
  gSite.body.append(
    ctrl('Latitude', latIn.el, { unit: '°N' }),
    ctrl('Elevation', elevIn.el, { unit: 'm' }),
    el('div', { class: 'hint', style: { marginTop: '0.2rem' } }, 'Latitude drives solar geometry (Penman-Monteith, Hargreaves, Priestley-Taylor); elevation sets air pressure. Not every method needs both.'),
  );
