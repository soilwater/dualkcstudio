...
/**
 * modes/single-season/docs.js — documentation shown in the drawer while
...
<ol>
  <li><strong>Weather &amp; site</strong> — drop a daily CSV or load the example
      dataset. The file must already have an <code>eto</code> column (see
      the format below).</li>
...
<table>
  ...
  <tr><td><code>date</code></td><td>YYYY-MM-DD</td><td>required</td></tr>
  <tr><td><code>prcp</code></td><td>precipitation, mm</td><td>required</td></tr>
  <tr><td><code>eto</code></td><td>reference ET, mm/day</td><td>required</td></tr>
  <tr><td><code>wspd</code></td><td>wind speed at 2 m, m/s</td><td>optional</td></tr>
  <tr><td><code>rmin</code></td><td>min relative humidity, %</td><td>optional</td></tr>
</table>
<p>Don't have <code>eto</code>? Compute it from raw weather in the
<strong>ETo Calculator</strong>, then upload the file it hands back (your
data with an <code>eto</code> column appended). <code>wspd</code> and
<code>rmin</code> feed the FAO-56 wind/humidity corrections (Kc_max Eq. 72,
the Eq. 70 Kcb adjustment); when absent, the standard climate
(2 m/s, 45 %) is used and the climate-adjust option is disabled.</p>
<p>Other optional columns: <code>irrig</code> (applied irrigation, used by
the “Data” irrigation mode) and <code>kcb_obs</code> (sparse observed
Kcb, e.g. from remote sensing — it overrides the curve where present and is
interpolated between observations). Short interior gaps in <code>eto</code>,
<code>wspd</code> and <code>rmin</code> are filled without extrapolation.</p>
...
<table>
  ...
  <tr><td>Data</td><td>applies the CSV's <code>irrig</code> column as-is</td></tr>
...
ratio — a water-limited estimate, not a full crop model.</div>
