...
/**
 * modes/soil-limits/index.js — a lookup: pick a texture class and a source,
...
function create() {
  ...
  function render() {
    ...
    noteEl.textContent = srcMeta ? srcMeta.note : '';
    naEl.innerHTML = '';
    if (!rec) {
      metrics.update([]);
    ...
    const tiles = [{ label: 'Wilting point', value: rec.wp, digits: 3, unit: 'm³/m³' }];
    if (rec.fc33 != null || rec.fc10 != null) {
      /* Retention source: field capacity at two matric potentials. */
      tiles.push({ label: 'Field capacity (−33 kPa)', value: rec.fc33, digits: 3, unit: 'm³/m³' });
      tiles.push({ label: 'Field capacity (−10 kPa)', value: rec.fc10, digits: 3, unit: 'm³/m³' });
    } else {
      tiles.push({ label: 'Field capacity', value: rec.fc, digits: 3, unit: 'm³/m³' });
      tiles.push({ label: 'Available water', accent: true, value: Math.max(0, rec.fc - rec.wp), digits: 3, unit: 'm³/m³', title: 'Field capacity − wilting point' });
    }
